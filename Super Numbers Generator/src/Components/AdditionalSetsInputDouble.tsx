...
type AdditionalSetsInputDoubleProps = {
...
export const AdditionalSetsInputDouble = ({gridVisibilitySwitch, rangeMinimum, rangeMaximum, amount, amountAdditional, minAdditional, maxAdditional}: AdditionalSetsInputDoubleProps) => {

    const [addButton, setAddButton] = useState<boolean>(false);
    const [selectedOption, setSelectedOption] = useState<number | null>(null);
    const [additionalSetsData, setAdditionalSetsData] = useState<number[][]>([]);
    const [additionalSecondarySetsData, setAdditionalSecondarySetsData] = useState<number[][]>([]);
    ...
    const handleAdditionalSetsData = (e: React.FormEvent) => {
        ...
        if(selectedOption) {
            const newSets = [];
            const newSecondarySets =[];

            for (let i = 0; i < selectedOption; i++) {
                const additionalSet = numbersGenerator(rangeMinimum, rangeMaximum, amount);
                const additionalSecondarySet = numbersGenerator(minAdditional, maxAdditional, amountAdditional);
                newSets.push(additionalSet);
                newSecondarySets.push(additionalSecondarySet);
            }

            setAdditionalSetsData(newSets);
            setAdditionalSecondarySetsData(newSecondarySets);
    ...
    return (
        <>
            ...
            {additionalSetsData.length > 0 && <motion.div className='double-set-container'>
                    ...
                    {additionalSetsData.map((element, index) => (
                        <motion.ul className='additionalSetsList'
                            ...
                            {element.map((item, index) => (
                                <motion.div className='additional-number-circle-double'
                                            key={index}
                ...
                <motion.ul className='additionalSets-container'>
                    {additionalSecondarySetsData.map((element, index) => (
                        <motion.ul className='additionalSetsList'
                                   key={index}
                                   variants={containerList}
                                   initial="hidden"
                                   animate="visible">
                            {element.map((item, index) => (
                                <motion.div className='additional-secondary-number-circle-double'
                                            key={index}
                                            variants={listItem}><p>{item}</p>
                                </motion.div>
                            ))}
